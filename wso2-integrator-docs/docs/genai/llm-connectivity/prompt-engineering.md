...
description: Best practices for writing effective prompts in integration scenarios with WSO2 Integrator.
---

# Prompt Engineering

Prompt engineering determines how well your agents, natural functions, and natural expressions perform. A well-crafted prompt turns an unpredictable LLM into a reliable integration component. This page covers practical techniques for writing prompts in the context of enterprise integrations.

Unlike chatbot prompts, integration prompts must produce consistent, structured, machine-parseable output that downstream systems can consume.
...
```ballerina
// Reusable prompt builder for classification tasks
...
function testTicketClassification() returns error? {
    // Test known inputs produce expected outputs
...
- [Model Selection](model-selection.md) -- Choose the right model for your prompts
- [Natural Expressions](natural-expressions.md) -- Use natural language inline in code
- [Natural Functions](/docs/genai/agents/natural-functions) -- Build reusable LLM-powered functions
- [Input/Output Guardrails](/docs/genai/guardrails/input-output-guardrails) -- Programmatic validation beyond prompt-level constraints
