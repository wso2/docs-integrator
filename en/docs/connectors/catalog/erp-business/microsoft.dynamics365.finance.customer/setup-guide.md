...
2. Give the application a name (e.g., `d365-finance-customer-connector`), select the appropriate supported account type, and select **Register**.
...
3. Enter a **User name** and **User ID** for the application (e.g., `CUSTOMER-APP`).
...
5. Assign one or more **Security roles** that grant the access the connector needs — for example, a role with read/write access to **Accounts receivable → Customers**, customer groups, and customer parameters.
