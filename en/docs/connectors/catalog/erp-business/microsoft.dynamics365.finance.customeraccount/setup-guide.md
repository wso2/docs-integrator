...
- A Microsoft Dynamics 365 Finance & Operations environment (cloud-hosted, sandbox, or on-premises) with the Accounts receivable module enabled.
...
2. Give the application a name (e.g., `d365-finance-customeraccount-connector`), select the appropriate supported account type, and select **Register**.
...
3. Enter a **User name** and **User ID** for the application (e.g., `CUSTACCOUNT-APP`).
...
5. Assign one or more **Security roles** that grant the access the connector needs — for example, a role with read/write access to **Accounts receivable → Customers → Customer accounts**, covering electronic addresses, postal addresses, payment methods, payment journals, and posting profiles.
