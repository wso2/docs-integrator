...
- A Microsoft Dynamics 365 Finance & Operations environment (cloud-hosted, sandbox, or on-premises) with the General ledger module enabled.
...
2. Give the application a name (e.g., `d365-finance-mainaccount-connector`), select the appropriate supported account type, and select **Register**.
...
3. Enter a **User name** and **User ID** for the application (e.g., `MAINACCOUNT-APP`).
...
5. Assign one or more **Security roles** that grant the access the connector needs — for example, a role with read/write access to **General ledger → Chart of accounts → Main accounts**, ledgers, ledger account groups, and asset leasing (for lease books).
