...
- A Microsoft Dynamics 365 Finance & Operations environment (cloud-hosted, sandbox, or on-premises) with the Cash and bank management module enabled.
...
2. Give the application a name (e.g., `d365-finance-cashmanagement-connector`), select the appropriate supported account type, and select **Register**.
...
3. Enter a **User name** and **User ID** for the application (e.g., `CASHMGMT-APP`).
...
5. Assign one or more **Security roles** that grant the access the connector needs — for example, a role with read/write access to **Cash and bank management → Bank accounts**, **Cash accounts**, and the related setup pages for bank groups, cash discounts, cash ledger posting profiles, cash symbols, CODA transaction codes, and currency exchange rate types.
