...
This guide walks you through configuring an SAP S/4HANA Communication Arrangement to expose the Condition Record for Pricing in Sales API and obtaining the credentials required by the connector.
...
4. Enter a **User Name** (e.g., `PRICING_COND_USER`) and a **Description**.
...
2. Select **New** and enter a **System ID** (e.g., `BALLERINA_PRICING_COND`) and a **System Name**.
3. Under **Technical Data**, set the **Host Name** to the hostname of your client or integration platform.
4. Under **Users for Inbound Communication**, select **+** and select the communication user you created in the previous step.
5. Select **Save**.
...
2. Select **New** and search for the communication scenario that includes the Condition Record for Pricing in Sales API
   (**SAP_COM_0294: Pricing Data Integration**).
3. Select the scenario, then select **Create**.
...
:::note
The correct communication scenario may vary by SAP S/4HANA release and system configuration. Consult your SAP Basis administrator to confirm which scenario exposes the API_SLSPRICINGCONDITIONRECORD_SRV service.
:::
...
3. Locate the entry for **API_SLSPRICINGCONDITIONRECORD_SRV** and note the **Service URL**.
   It follows the pattern:
   ```
   https://<your-tenant>-api.s4hana.cloud.sap/sap/opu/odata/sap/API_SLSPRICINGCONDITIONRECORD_SRV
   ```
