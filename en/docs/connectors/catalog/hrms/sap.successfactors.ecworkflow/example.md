...
Build an integration that connects to SAP SuccessFactors Employee Central using the SAP SuccessFactors Employee Central Workflow connector and retrieves MyPendingWorkflow records. The integration sets up an automation entry point that invokes the operation and returns the result.

**Operations used:**
- **listMyPendingWorkflows** : Queries the MyPendingWorkflow collection and returns a page of entities, optionally filtered, sorted, and paged via OData query options.

## Architecture

```mermaid
flowchart LR
    A((User)) --> B[listMyPendingWorkflows]
...
### Step 1: Open the connector palette and add the SAP SuccessFactors Employee Central Workflow connector

1. From the project canvas, select **+ Add Artifact** → **Connection**.
2. In the connector search palette, enter `sap.successfactors.ecworkflow`.
3. Locate **ballerinax/sap.successfactors.ecworkflow** and select **Add**.
...
- **connectionName** : Logical name for this connection — leave the default value `ecworkflowClient`

### Step 3: Save the connection

Select **Save Connection**. The connection is saved and `ecworkflowClient` appears in the **Connections** section of the project tree.
...
## Configuring the SAP SuccessFactors Employee Central Workflow listMyPendingWorkflows operation
...
### Step 6: Select and configure the listMyPendingWorkflows operation

1. In the automation flow canvas, select the **+** button between the **Start** node and the **Error Handler** node.
2. In the step-addition panel, expand **ecworkflowClient** under the **Connections** section.
3. Select **listMyPendingWorkflows**.
