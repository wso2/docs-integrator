...
Build an integration that connects to SAP SuccessFactors Employee Central using the SAP SuccessFactors Employee Central Employee Profile connector and retrieves Background_Community records. The integration sets up an automation entry point that invokes the operation and returns the result.

**Operations used:**
- **listBackgroundCommunities** : Queries the Background_Community collection and returns a page of entities, optionally filtered, sorted, and paged via OData query options.

## Architecture

```mermaid
flowchart LR
    A((User)) --> B[listBackgroundCommunities]
...
### Step 1: Open the connector palette and add the SAP SuccessFactors Employee Central Employee Profile connector

1. From the project canvas, select **+ Add Artifact** → **Connection**.
2. In the connector search palette, enter `sap.successfactors.ecemployeeprofile`.
3. Locate **ballerinax/sap.successfactors.ecemployeeprofile** and select **Add**.
...
- **connectionName** : Logical name for this connection — leave the default value `ecemployeeprofileClient`

### Step 3: Save the connection

Select **Save Connection**. The connection is saved and `ecemployeeprofileClient` appears in the **Connections** section of the project tree.
...
## Configuring the SAP SuccessFactors Employee Central Employee Profile listBackgroundCommunities operation
...
### Step 6: Select and configure the listBackgroundCommunities operation

1. In the automation flow canvas, select the **+** button between the **Start** node and the **Error Handler** node.
2. In the step-addition panel, expand **ecemployeeprofileClient** under the **Connections** section.
3. Select **listBackgroundCommunities**.
