...
Build an integration that connects to SAP SuccessFactors Employee Central using the SAP SuccessFactors Employee Central Dismissal Protection connector and retrieves EmployeeDismissalProtectionDetail records. The integration sets up an automation entry point that invokes the operation and returns the result.

**Operations used:**
- **listEmployeeDismissalProtectionDetails** : Queries the EmployeeDismissalProtectionDetail collection and returns a page of entities, optionally filtered, sorted, and paged via OData query options.

## Architecture

```mermaid
flowchart LR
    A((User)) --> B[listEmployeeDismissalProtectionDetails]
...
### Step 1: Open the connector palette and add the SAP SuccessFactors Employee Central Dismissal Protection connector

1. From the project canvas, select **+ Add Artifact** → **Connection**.
2. In the connector search palette, enter `sap.successfactors.ecdismissalprotection`.
3. Locate **ballerinax/sap.successfactors.ecdismissalprotection** and select **Add**.
...
- **connectionName** : Logical name for this connection — leave the default value `ecdismissalprotectionClient`

### Step 3: Save the connection

Select **Save Connection**. The connection is saved and `ecdismissalprotectionClient` appears in the **Connections** section of the project tree.
...
## Configuring the SAP SuccessFactors Employee Central Dismissal Protection listEmployeeDismissalProtectionDetails operation
...
### Step 6: Select and configure the listEmployeeDismissalProtectionDetails operation

1. In the automation flow canvas, select the **+** button between the **Start** node and the **Error Handler** node.
2. In the step-addition panel, expand **ecdismissalprotectionClient** under the **Connections** section.
3. Select **listEmployeeDismissalProtectionDetails**.
