...
Build an integration that connects to SAP SuccessFactors Employee Central using the SAP SuccessFactors Employee Central Payment Information connector and retrieves PaymentInformationDetailV3KEN records. The integration sets up an automation entry point that invokes the operation and returns the result.

**Operations used:**
- **listPaymentInformationDetailV3KENs** : Queries the PaymentInformationDetailV3KEN collection and returns a page of entities, optionally filtered, sorted, and paged via OData query options.

## Architecture

```mermaid
flowchart LR
    A((User)) --> B[listPaymentInformationDetailV3KENs]
...
### Step 1: Open the connector palette and add the SAP SuccessFactors Employee Central Payment Information connector

1. From the project canvas, select **+ Add Artifact** → **Connection**.
2. In the connector search palette, enter `sap.successfactors.ecpaymentinformation`.
3. Locate **ballerinax/sap.successfactors.ecpaymentinformation** and select **Add**.
...
- **connectionName** : Logical name for this connection — leave the default value `ecpaymentinformationClient`

### Step 3: Save the connection

Select **Save Connection**. The connection is saved and `ecpaymentinformationClient` appears in the **Connections** section of the project tree.
...
## Configuring the SAP SuccessFactors Employee Central Payment Information listPaymentInformationDetailV3KENs operation
...
### Step 6: Select and configure the listPaymentInformationDetailV3KENs operation

1. In the automation flow canvas, select the **+** button between the **Start** node and the **Error Handler** node.
2. In the step-addition panel, expand **ecpaymentinformationClient** under the **Connections** section.
3. Select **listPaymentInformationDetailV3KENs**.
