...
Build an integration that connects to SAP SuccessFactors Employee Central using the SAP SuccessFactors Employee Central Skills Management connector and retrieves CertificationContent records. The integration sets up an automation entry point that invokes the operation and returns the result.

**Operations used:**
- **listCertificationContents** : Queries the CertificationContent collection and returns a page of entities, optionally filtered, sorted, and paged via OData query options.

## Architecture

```mermaid
flowchart LR
    A((User)) --> B[listCertificationContents]
...
### Step 1: Open the connector palette and add the SAP SuccessFactors Employee Central Skills Management connector

1. From the project canvas, select **+ Add Artifact** → **Connection**.
2. In the connector search palette, enter `sap.successfactors.ecskillsmanagement`.
3. Locate **ballerinax/sap.successfactors.ecskillsmanagement** and select **Add**.
...
- **connectionName** : Logical name for this connection — leave the default value `ecskillsmanagementClient`

### Step 3: Save the connection

Select **Save Connection**. The connection is saved and `ecskillsmanagementClient` appears in the **Connections** section of the project tree.
...
## Configuring the SAP SuccessFactors Employee Central Skills Management listCertificationContents operation
...
### Step 6: Select and configure the listCertificationContents operation

1. In the automation flow canvas, select the **+** button between the **Start** node and the **Error Handler** node.
2. In the step-addition panel, expand **ecskillsmanagementClient** under the **Connections** section.
3. Select **listCertificationContents**.
