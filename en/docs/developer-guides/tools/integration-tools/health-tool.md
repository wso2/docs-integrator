...
???+ Note
    It is recommended to use a Standard for Trial Use (STU) or higher release level of the implementation guides. Make sure that the downloaded specification archive has the `StructureDefinition`, `ValueSet`, and `CodeSystem` files for the Implementation Guide (IG) resources when extracted.
...
???+ Note
    Make sure to give the directory path of the downloaded FHIR definitions as the last argument.
...
4. Push it to a repository.
    ...
    ???+ Tip
        You can push either to the local repository or to [Ballerina Central](https://central.ballerina.io/), which is a remote repository.
...
???+ Note
    You need to have a Ballerina package containing IG-specific FHIR resource data models to generate FHIR IG templates. You can use the package mode of the Health tool for easy generation of this package. This example uses the [`health.fhir.r4.uscore501`](https://central.ballerina.io/ballerinax/health.fhir.r4.uscore501/latest) package in Ballerina Central.
...
#### Step 3: Use the generated templates
...
#### Step 1: Create the CDS definition file
