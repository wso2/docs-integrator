...
This walkthrough uses plain **FTP** so the visual steps stay simple and reproducible. For real partner exchange, switch the listener to **SFTP** or **FTPS** — same services and handlers, just a different protocol and credentials on the listener. See [Creating an SFTP service](../../develop/integration-artifacts/file/ftp-sftp.md#creating-an-sftp-service).
:::
...
- A running **FTP server** with the directories `/greenfield`, `/harbor`, `/processed`, and `/errors`, plus a username/password.
- A running **MySQL** instance with the `order_hub` database and the `orders` and `order_lines` tables (DDL in [Step 1](#step-1-set-up-the-project-and-the-canonical-model)).
- An **SMTP server** to receive the summary email. A local mock such as [Mailpit](https://github.com/axllent/mailpit) works well for testing (SMTP on `1025`, web inbox on `8025`).
:::
...
Everything you build lives in one project that holds both integrations, so you set that up first — together with the two things every later step leans on: the **canonical `Order` model** the suppliers' formats normalize into, and the **MySQL database** that stores each order. The data mapper in [Step 3](#step-3-greenfield--map-csv-to-the-canonical-order) and [Step 4](#step-4-harbor--map-xml-to-the-same-canonical-order) maps both CSV and XML onto this one model, so defining it now is what lets the rest of the hub converge.

### Create the database

The hub persists orders in MySQL, so create the schema before you connect to it. Run this script against your MySQL instance. It creates the database, the application user the integration connects as, and the two tables: `orders` and its child `order_lines`, linked by a foreign key so an order's lines are deleted with it.
...
You now have an `order_hub` database reachable with the credentials below. You give these to the connection later in this step:
...
In `orders`, `order_id` is the supplier's **business identifier** (a string such as `GF-10231`). In `order_lines`, `order_id` is the **integer foreign key** into `orders.id`. The persist client surfaces this difference for you — the line's foreign key takes the generated integer key, not the business id — but it is worth keeping straight as you read the inserts in [Step 3](#step-3-greenfield--map-csv-to-the-canonical-order).
:::
...
3. Add the database connection. In the artifacts panel, open **Connections**, click **+**, and choose [**Connect to a Database**](../../develop/tools/integration-tools/persist-tool.md#step-1-add-a-connection). Select **MySQL** and enter the credentials from the table above, then click **Connect & Introspect Database**.
...
## Step 3: Greenfield — map CSV to the canonical `Order`
...
## Step 4: Harbor — map XML to the same canonical `Order`

Harbor sends the same kind of order, but as **XML** — with the values held in attributes, the numbers typed as text, and the date written `DD/MM/YYYY`. The point of this step is that none of that difference survives into the database: the XML lands on the **exact same canonical `Order`** as the CSV, through its own mapper and the same insert shape. You generate the source type from a sample, map it, and persist it just as in [Step 3](#step-3-greenfield--map-csv-to-the-canonical-order).
...
<!-- ☐ Build: integration `daily-summary` + its own `dbClient`; OrderSummaryType;
     automation: read today's orders (parameterized whereClause + time:Date), email via SMTP. -->

_Intent: a scheduled job, separate from intake, that reads the day's orders and emails a report._

<Tabs>
<TabItem value="ui" label="Visual Designer" default>

1. _Add a second integration `daily-summary` with its own `dbClient` connection._
   > 📸 **Screenshot:** Add integration — `order-hub/summary-create-integration.png`
2. _Add the `OrderSummaryType` type and the Email (SMTP) connection (`emailSmtpclient`)._
   > 📸 **Screenshot:** Email connection config — `order-hub/summary-email-connection.png`
3. _Add the **Automation**: read today's orders by date, then send the summary email._
   > 📸 **Screenshot:** automation flow — `order-hub/summary-flow.png`

</TabItem>
<TabItem value="code" label="Ballerina Code">

<!-- ☑ Available: daily_summary/automation.bal + OrderSummaryType + email connection. -->
```ballerina
// automation.bal — read today's orders + email summary (to be inserted)
```

</TabItem>
</Tabs>

> **Capability: scheduled batch with connector delivery.** _Callout: the persist client that stored each order now feeds a daily report, queried by date and emailed, on a timer rather than a request._

---

## Step 6: Run and test

<!-- ☐ Build: bring up FTP + MySQL + mock SMTP; drop GF_*.csv/.ok and HS_*.xml/.ok;
     watch files move to /processed; verify orders/order_lines; run automation; see mail in Mailpit. -->

_Intent: exercise both intakes and the report end to end._

1. _Start the servers and fill each integration's `Config.toml`; **Run** both integrations._
2. _Drop a Greenfield CSV (+ `.ok`) and a Harbor XML (+ `.ok`); watch each move to `/processed`._
   > 📸 **Screenshot:** directories before/after — `order-hub/run-dirs.png`
3. _Confirm both orders landed in MySQL in the **same canonical shape**._
   > 📸 **Screenshot:** orders / order_lines query — `order-hub/db-result.png`
4. _Drop a malformed file; confirm it lands in `/errors`._
5. _Run the `daily-summary` automation; confirm the report email arrives._
   > 📸 **Screenshot:** Mailpit inbox with the report — `order-hub/mail-result.png`
